...
export class AccountEndpoint extends EndpointBase {
  ...
  get usersToAssignTestUrl() { return this.configurations.baseUrl + '/api/account/allUsers'; }
  ...
  getUsersToAssignTestEndpoint<T>(): Observable<T> {
    const endpointUrl = this.usersToAssignTestUrl;
    return this.http.get<T>(endpointUrl, this.requestHeaders).pipe<T>(
      catchError(error => {
        return this.handleError(error, () => this.getUsersToAssignTestEndpoint());
      }));
  }
